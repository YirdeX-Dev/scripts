...
local Wjgjgn = Tabs.Main:AddLeftGroupbox("全游戏通用攻击", "sword")
...
local Ksqcnbcos = Tabs.Main:AddRightGroupbox("透视（全部可在墨水使用）", "eye")
...
LeftGroupBox:AddInput("VictimInput", {
    Default = "cmwk222",
    Text = "目标玩家名字",
    Placeholder = "输入目标名字...",
...
local function applyDisguise(victimName)
    if victimName == "" then
        Library:Notify({ Title = "错误", Description = "请输入一个目标玩家的名字", Time = 3 })
        return false
    end

    if isWhitelisted(victimName) then
        Library:Notify({ Title = "禁止操作", Description = "无法伪装白名单玩家", Time = 3 })
        return false
    end
    ...
    local success = pcall(function()
        ...
        if not userId then
            Library:Notify({ Title = "错误", Description = "找不到该玩家", Time = 3 })
            return
        end

        shared.FakeDisguiseActive = true

        local lp = Players.LocalPlayer
        lp.Name = userName
        lp.UserId = userId
        lp.CharacterAppearanceId = userId
        ...
        if char:FindFirstChildOfClass("Humanoid") then
            char.Humanoid.DisplayName = displayName
        end

        applyCharacterAppearance(char, userId)

        Library:Notify({
            Title = "成功",
            Description = "伪装为: " .. displayName,
            Time = 3,
        })
    end)
...
LeftGroupBox:AddButton({
    Text = "应用伪装",
    Func = function()
        if applyDisguise(Options.VictimInput.Value) then
            lastVictimName = Options.VictimInput.Value
        end
    end,
})
...
LeftGroupBox:AddButton({
    ...
    Func = function()
        ...
        Library:Notify({ Title = "已重置", Description = "伪装已清除", Time = 2 })
    end,
})

-- ===== 修改他人外观 =====
LeftGroupBox:AddDivider()
LeftGroupBox:AddLabel("修改他人外观")

LeftGroupBox:AddDropdown('TargetPlayerList', {
    SpecialType = 'Player',
    Text = '选择要修改的玩家',
})

LeftGroupBox:AddInput("AppearanceSourceInput", {
    Text = "外观来源玩家名字",
    Placeholder = "输入外观来源玩家名字...",
})

local function getSelectedTargetPlayer()
    local v = Options.TargetPlayerList.Value
    return (v and typeof(v) == "Instance" and v:IsA("Player")) and v or nil
end

local function applyAppearanceToTarget(appearanceName)
    local target = getSelectedTargetPlayer()
    if not target then
        Library:Notify({ Title = "错误", Description = "请选择一个目标玩家", Time = 3 })
        return false
    end

    if isWhitelisted(target.Name) then
        Library:Notify({ Title = "禁止操作", Description = "无法修改白名单玩家", Time = 3 })
        return false
    end

    if isWhitelisted(appearanceName) then
        Library:Notify({ Title = "禁止操作", Description = "不能使用白名单玩家外观", Time = 3 })
        return false
    end

    local userId, userName, displayName = getUserIdByUsername(appearanceName)
    if not userId then
        Library:Notify({ Title = "错误", Description = "找不到外观来源玩家", Time = 3 })
        return false
    end
    ...
    target.CharacterAppearanceId = userId
    ...
    shared.FakeTargetDisguise[target] = true

    Library:Notify({
        Title = "成功",
        Description = string.format("已将 %s 伪装为 %s", target.Name, displayName),
        Time = 3,
    })
end
...
LeftGroupBox:AddButton({
    Text = "修改他人外观和名字",
    Func = function()
        applyAppearanceToTarget(Options.AppearanceSourceInput.Value)
    end,
})
...
LeftGroupBox:AddButton({
    ...
    Func = function()
        ...
        if isWhitelisted(target.Name) then
            Library:Notify({ Title = "禁止操作", Description = "无法重置白名单玩家", Time = 3 })
            return
        end

        shared.FakeTargetDisguise[target] = nil
        target:LoadCharacter()
        Library:Notify({ Title = "已重置", Description = "目标外观已恢复", Time = 2 })
    end,
})
...
LeftGroupBox:AddButton({
    Text = "刷新玩家列表",
    Func = function()
        if Options.TargetPlayerList.Refresh then
            Options.TargetPlayerList:Refresh()
        end
    end,
})
local RunService = game:GetService("RunService")
...
local scriptCategories = {
    {
        GroupName = "各大脚本（通用功能）",
        List = {
            {Name = "AF脚本", Url = "https://raw.githubusercontent.com/YirdeX-Dev/scripts/refs/heads/main/YX-HubLoader.lua"},
...
Tabs.Afhubyyds:UpdateWarningBox({
        Title = '付费信息',
        Text = '付费功能都意思不是需要付费才能使用\n是收集来的付费脚本功能\n[<font color=\"rgb(255, 0, 0)\">有些功能无法使用 谨慎使用！</font>]',
        IsNormal = true,
        Visible = true,
        LockSize = true,
    })
local Afhubyyds = Tabs.Afhubyyds:AddLeftGroupbox("缝合脚本区(缝合功能)", "link")
Afhubyyds:AddLabel("服务器功能使用情况：")
Afhubyyds:AddLabel("已添加撕咬之夜功能🟢")
Afhubyyds:AddLabel("已添加九十九夜功能🟡")
Afhubyyds:AddLabel("已添加恐鬼症功能🟢")
Afhubyyds:AddLabel("已添加力量传奇功能🟡")
...
local function toggleAutoDoor(state)
...
local function toggleInfiniteStamina(state)
    ...
    if infiniteStaminaEnabled then
        ...
        infiniteStaminaConnection = RunService.Heartbeat:Connect(function()
            if LocalPlayer.Character then
                LocalPlayer.Character:SetAttribute('WalkSpeed', 25)
            end
        end)
    else
        if infiniteStaminaConnection then infiniteStaminaConnection:Disconnect() end
        if LocalPlayer.Character then
            LocalPlayer.Character:SetAttribute('WalkSpeed', 16)
        end
    end
end

local function toggleRunSpeed(state)
    runSpeedEnabled = state
    if runSpeedEnabled and LocalPlayer.Character then
        LocalPlayer.Character:SetAttribute('RunSpeed', runSpeedValue)
    end
end

local function setRunSpeed(value)
    runSpeedValue = value
    if runSpeedEnabled and LocalPlayer.Character then
        LocalPlayer.Character:SetAttribute('RunSpeed', runSpeedValue)
    end
...
local function toggleWalkSpeed(state)
    ...
    if walkSpeedEnabled and LocalPlayer.Character then
        LocalPlayer.Character:SetAttribute('WalkSpeed', walkSpeedValue)
    end
end
...
local function toggleAutoRepair(state)
            ...
            while autoRepairEnabled do
                ...
                if generators then
                    for _, generator in pairs(generators:GetChildren()) do
                        if generator:IsA('Model') and generator:GetAttribute('broken') == true then
                            local repairPrompt = generator:FindFirstChild('ProximityPrompt')
                            if repairPrompt then
                                fireproximityprompt(repairPrompt)
                            end
                        end
                    end
                end
...
local function setAutoRepairDelay(value)
    ...
    if emotePlaying then
        ...
        emoteConnection = RunService.Heartbeat:Connect(function()
            if selectedEmote then
                local ReplicatedStorage = game:GetService("ReplicatedStorage")
                local emoteModule = ReplicatedStorage:FindFirstChild('Modules'):FindFirstChild('Emotes'):FindFirstChild(selectedEmote)
                if emoteModule then
                    local emoteId = require(emoteModule)
                    if emoteId and LocalPlayer.Character then
                        local humanoid = LocalPlayer.Character:FindFirstChild('Humanoid')
                        if humanoid then
                            local animTrack = humanoid:LoadAnimation(Instance.new('Animation'))
                            animTrack.AnimationId = emoteId
                            animTrack:Play()
                        end
                    end
                end
...
local highlights = {}
for i = 1, 20 do
    local color = highlightColors[(i % #highlightColors) + 1]
...
local Afhubyyds = Tabs.Afhubyyds:AddLeftGroupbox("撕咬之夜功能", "moon")

Afhubyyds:AddToggle("BypassAntiCheat", {
    Text = "绕过反作弊",
    Default = false,
    Callback = function(state)
        if state then
...
Afhubyyds:AddToggle("AutoDoor", {
    Text = "自动挡门",
    Default = false,
    Callback = toggleAutoDoor
})

Afhubyyds:AddToggle("Noclip", {
    Text = "穿墙",
    Default = false,
    Callback = toggleNoclip
})

Afhubyyds:AddToggle("Fullbright", {
    Text = "全亮",
    Default = false,
    Callback = toggleFullbright
})

Afhubyyds:AddButton("DeleteDoors", {
    Text = "删除所有门",
    Func = deleteAllDoors
})

Afhubyyds:AddToggle("InfiniteStamina", {
    Text = "无限体力+始终冲刺",
    Default = false,
    Callback = toggleInfiniteStamina
})

Afhubyyds:AddSlider("RunSpeed", {
    Text = "跑步速度",
    Default = 24,
    Min = 0,
    Max = 50,
    Rounding = 0,
    Callback = setRunSpeed
})

Afhubyyds:AddToggle("EnableRunSpeed", {
    Text = "启用跑步速度",
    Default = false,
    Callback = toggleRunSpeed
})

Afhubyyds:AddSlider("WalkSpeed", {
    Text = "行走速度",
    Default = 15,
    Min = 0,
    Max = 50,
...
Afhubyyds:AddToggle("EnableWalkSpeed", {
    Text = "启用行走速度",
    Default = false,
    Callback = toggleWalkSpeed
})

Afhubyyds:AddToggle("AutoRepair", {
    Text = "自动修机",
    Default = false,
    Callback = toggleAutoRepair
})

Afhubyyds:AddSlider("RepairDelay", {
    Text = "修机延迟",
    Default = 0.5,
    Min = 0.1,
    Max = 15,
    Rounding = 1,
    Callback = setAutoRepairDelay
})

Afhubyyds:AddToggle("AutoBlock", {
    Text = "自动格挡",
    Default = false,
    Callback = toggleAutoBlock
})

Afhubyyds:AddToggle("KillAll", {
    Text = "杀死所有人",
    Default = false,
    Callback = toggleKillAll
})

Afhubyyds:AddSlider("HitboxSize", {
    Text = "碰撞箱大小",
    Default = 15,
    Min = 0,
    Max = 30,
...
Afhubyyds:AddToggle("EnableHitbox", {
    Text = "启用碰撞箱",
    Default = false,
    Callback = toggleHitbox
})

Afhubyyds:AddButton("EnableHighlights", {
    Text = "启用高亮",
    Func = function()
        toggleAllHighlights(true)
    end
})

Afhubyyds:AddButton("DisableHighlights", {
    Text = "禁用高亮",
    Func = function()
...
local Afhubyyds = Tabs.Afhubyyds:AddRightGroupbox("预计下次更新功能(预告)", "hourglass")
Afhubyyds:AddLabel("待添加：不会再有了")
Afhubyyds:AddLabel("公告：功能不是每个有效 谨慎使用小心封禁！")
...
local Afhubyyds = Tabs.Afhubyyds:AddLeftGroupbox("恐鬼症功能", "ghost")
Afhubyyds:AddButton({
    Text = "透视/夜视",
    Func = function()
...
Afhubyyds:AddToggle("DaylightChat", {
    Text = "白天可见(聊天窗)",
    Default = false,
    Callback = function(Value)
...
Afhubyyds:AddToggle("AutoSpiritBox", {
    Text = "高频精灵盒对话",
    Default = false,
    Callback = function(Value)
        if Value then
...
Afhubyyds:AddToggle("IgnoreGhost", {
    Text = "透视幽灵仇恨",
    Tooltip = "使用该功能会让幽灵无视你(请不要在房子内开启)",
    Default = false,
    Callback = function(Value)
        if LocalPlayer:FindFirstChild("Zone") and LocalPlayer.Zone.Value ~= "Outside" and Value == true then
...
Afhubyyds:AddToggle("RemoteInteract", {
    Text = "远程互动",
    Default = false,
    Callback = function(Value)
        if Value then
            if fireproximityprompt then
                ProximityConn = ProximityPromptService.PromptButtonHoldBegan:Connect(function(prompt)
                    fireproximityprompt(prompt)
                end)
            else
                print("你的注入器不支持此命令(fireproximityprompt)")
            end
...
Afhubyyds:AddButton({
    ...
    Func = function()
        ...
        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Photo Camera") and LocalPlayer.Character:FindFirstChild("Head") then
...
Afhubyyds:AddButton({
    Text = "凭空放盐",
    Func = function()
        if GhostRoomPart and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Salt") then
            LocalPlayer.Character.Salt.Remote.Drop:FireServer(unpack({
                [1] = GhostRoomPart.CFrame,
                [2] = LocalPlayer.Character.Salt.Ammo.Capacity,
            }))
        else
            print("请确保角色手持了盐，且已探测到鬼房(最低温度)")
        end
    end
})
...
Afhubyyds:AddButton({
    Text = "血月加成",
    Func = function()
        if ReplicatedStorage.Remotes:FindFirstChild("BloodMoonEvent") then
            ReplicatedStorage.Remotes.BloodMoonEvent:FireServer()
        end
    end
})

Afhubyyds:AddSlider("CandleDistance", {
    Text = "蜡烛互动距离",
    Default = 4,
    Min = 4,
    Max = 10,
    ...
    Callback = function(Value)
        ...
        if Candles then
            for _, v in ipairs(Candles:GetDescendants()) do
...
Afhubyyds:AddToggle("CrashMatch", {
    Text = "炸对局",
    ...
    Callback = function(Value)
        if Value then
            ...
            if EnableAdMsg then
                TextChatService.TextChannels.RBXGeneral:SendAsync("Yttrium Hub[炸对局]功能开启:" .. RandomString(10))
            end
            ClearPictures()
            task.wait(2)
            CrashConn = RunService.Stepped:Connect(function()
                local camera = Workspace.Map.Items:FindFirstChild("Photo Camera")
                if camera and camera:FindFirstChild("Remote") then
                    camera.Remote.FireCameraEvent:FireServer(LocalPlayer.Character and LocalPlayer.Character.HumanoidRootPart.CFrame or CFrame.new(), {})
                end
            end)
        ...
        for _, v in next, workspace.GlassBridge.GlassHolder:GetChildren() do
            for _, panelChild in next, v:GetChildren() do
                for _, glassPart in next, panelChild:GetChildren() do
                    local highlight = glassPart:FindFirstChild("GlassESP")
                    if highlight then
                        highlight:Destroy()
                    end
                end
...
local Afhubyyds = Tabs.Afhubyyds:AddLeftGroupbox("九十九夜功能", "crown")
Afhubyyds:AddToggle('MyToggle', {
    Text = '无限血',
    Default = false,
    Tooltip = '无限的血量',
    Callback = function(Value)
...
local Speed = 1
local speedConn
Afhubyyds:AddSlider('SpeedSlider', {
    Text = '速度设置',
    Default = 1,
    Min = 1,
    Max = 150,
...
Afhubyyds:AddToggle('SpeedToggle', {
    Text = '速度 (开/关)',
    Default = false,
    Tooltip = '开启后提升移动速度',
    Callback = function(v)
...
local Part
local StoredCF
Afhubyyds:AddToggle('InvisToggle', {
    Text = '隐身',
    Default = false,
    Tooltip = '开启后角色隐形',
    Callback = function(Value)
...
Afhubyyds:AddDropdown('AttackModeDropdown', {
    Title = "攻击方式",
    Values = { "范围", "全图" },
    Default = 1,
    Multi = false,
...
Afhubyyds:AddInput('ChopTreeRangeInput', {
    Default = tostring(DistanceForAutoChopTree),
    Numeric = true,
    Finished = true,
    Text = '砍树范围 [500最佳]',
    Placeholder = '输入距离...',
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
            DistanceForAutoChopTree = num
            print("[回调] 砍树范围设置为:", num)
        else
            warn("请输入有效数字")
        end
    end
...
local DistanceForAutoChopTree = 500

Afhubyyds:AddToggle('AutoChopTreeToggle', {
    Text = '自动砍树',
    Default = false,
    Tooltip = '开启后自动寻找树木并破坏',
    Callback = function(Value)
        ...
        if Value then
            task.spawn(function()
                while ActiveAutoChopTree do
                    ...
                    if not weapon then
                        weapon = player.Inventory:FindFirstChild("Old Axe")
...
Afhubyyds:AddToggle('AutoChestToggle', {
    Text = '自动开全部宝箱',
    Default = false,
    Tooltip = '自动遍历地图打开所有宝箱',
    Callback = function(v)
        ...
        if v then
            ...
            local character = player.Character or player.CharacterAdded:Wait()
            local hrp = character:WaitForChild("HumanoidRootPart")
            chestSettings.originalCFrame = hrp.CFrame
            ...
            task.spawn(function()
                ...
                if chestSettings.originalCFrame then
                    hrp.CFrame = chestSettings.originalCFrame
                end
...
Afhubyyds:AddSlider('ChestRangeSlider', {
    Text = '宝箱光环范围',
    Min = 1,
    Max = 100,
    Default = 50,
    Callback = function(v)
        chestSettings.chestRange = v
    end
})
Afhubyyds:AddToggle('ChestAuraToggle', {
    Text = '宝箱光环',
    Default = false,
    Tooltip = '靠近自动开启宝箱',
    Callback = function(v)
            ...
            task.spawn(function()
                while chestSettings.autoChestNearEnabled do
                    ...
                    if hrp then
                        for _, chest in ipairs(getChests()) do
                            local part = chest.PrimaryPart or chest:FindFirstChildWhichIsA("BasePart")
                            if part and (hrp.Position - part.Position).Magnitude <= chestSettings.chestRange then
                                fireAllPrompts(chest)
                            end
                        end
                    end
...
Afhubyyds:AddButton({
    ...
    Func = function()
        ...
        if nearestPart then
            hrp.CFrame = nearestPart.CFrame + Vector3.new(0, nearestPart.Size.Y / 2 + 6, 0)
            Library:Notify("已传送到最近宝箱", 3)
        else
            Library:Notify("未找到宝箱", 3)
        end
    end
}):AddButton({
    Text = '强制开箱',
...
local function getMobs()
    ...
    for _, npc in ipairs(workspace:GetDescendants()) do
        if npc:IsA("Model") and npc.Name == "Lost Child" then
...
Afhubyyds:AddDropdown('MobDropdown', {
    Text = '选择儿童',
...
Afhubyyds:AddButton({
    Text = '刷新儿童列表',
    Func = function()
        local mobs, names = getMobs()
        currentMobNames = names

        if #names > 0 then
            selectedMob = names[1]
            Afhubyyds:GetDropdown('MobDropdown'):Refresh(names)
            Library:Notify("找到 " .. #names .. " 个迷失儿童", 3)
        else
            selectedMob = nil
            Afhubyyds:GetDropdown('MobDropdown'):Refresh({ "未找到迷失儿童" })
            Library:Notify("未找到迷失儿童", 3)
        end
    end
})
        ...
        for i, name in ipairs(names) do
            if name == selectedMob then
                ...
                if part and hrp then
                    ...
                    Library:Notify("已传送到 " .. selectedMob, 3)
                end
                break
            end
        end
...
local function bringItem(predicate)
    task.spawn(function()
        while autoStates[predicate] and task.wait() do
            local root = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if not root then continue end

            for _, item in ipairs(workspace.Items:GetChildren()) do
                local part = item:FindFirstChildWhichIsA("BasePart")
                if part and predicate(item) then
                    part.CFrame = root.CFrame * CFrame.new(
...
Afhubyyds:AddToggle('BringItemsToggle', {
    Text = '自动传送物品',
    Default = false,
    Callback = function(v)
        autoStates.Items = v
        bringItem(function() return true end)
    end
})

Afhubyyds:AddToggle('BringLogsToggle', {
    Text = '自动传送木头',
    Default = false,
    Callback = function(v)
        autoStates.Logs = v
        bringItem(function(item)
            return item.Name:lower():find("log")
        end)
    end
})

Afhubyyds:AddToggle('BringFuelToggle', {
    Text = '自动传送燃料罐',
    Default = false,
    Callback = function(v)
        autoStates.Fuel = v
        bringItem(function(item)
            return item.Name:lower():find("fuel canister")
        end)
    end
})

Afhubyyds:AddToggle('BringOilToggle', {
    Text = '自动传送油桶',
    Default = false,
    Callback = function(v)
        autoStates.Oil = v
        bringItem(function(item)
            return item.Name:lower():find("oil barrel")
        end)
    end
})

Afhubyyds:AddToggle('BringScrapToggle', {
    Text = '自动传送所有废料',
    Default = false,
    Callback = function(v)
        autoStates.Scrap = v
        bringItem(function(item)
            ...
            return name:find("tyre")
                or name:find("sheet metal")
                or name:find("broken fan")
                or name:find("bolt")
                or name:find("old radio")
                or name:find("ufo junk")
                or name:find("ufo scrap")
                or name:find("broken microwave")
        end)
    end
})

Afhubyyds:AddToggle('BringCoalToggle', {
    Text = '自动传送煤炭',
    Default = false,
    Callback = function(v)
        autoStates.Coal = v
        bringItem(function(item)
            return item.Name:lower():find("coal")
        end)
    end
})

Afhubyyds:AddToggle('BringMeatToggle', {
    Text = '自动传送肉类',
    Default = false,
    Callback = function(v)
        autoStates.Meat = v
        bringItem(function(item)
            ...
            return n:find("meat") or n:find("cooked")
        end)
    end
})
local itemMaps = {
...
for _, items in pairs(itemMaps) do
    for cnName, _ in pairs(items) do
        table.insert(allItemNames, cnName)
    end
end

Afhubyyds:AddDropdown('ItemCollectDropdown', {
    Text = '选择要收集的物品（多选）',
    Values = allItemNames,
    Default = {},
    Multi = true,
    ...
    Callback = function(Value)
        ...
        for itemName, enabled in pairs(Value) do
            if enabled then
                for _, map in pairs(itemMaps) do
                    if map[itemName] then
                        selectedItems[map[itemName]] = true
                        break
                    end
                end
            end
...
local function CollectItems()
    ...
    table.sort(itemsToCollect, function(a, b)
        local pa = a.PrimaryPart or a:FindFirstChildWhichIsA("BasePart")
        local pb = b.PrimaryPart or b:FindFirstChildWhichIsA("BasePart")
    ...
    for _, item in ipairs(itemsToCollect) do
        ...
        if part then
            ...
            game:GetService("ReplicatedStorage").RemoteEvents.ReplicateSound:FireServer(
                "FireAllClients",
                "BagGet",
                { Instance = player.Character.Head, Volume = 0.25 }
            )
...
Afhubyyds:AddToggle('AutoCollectToggle', {
    Text = '自动收集选择的物品',
    Default = false,
    Tooltip = '开启后将自动收集下拉框中选中的物品',
    Callback = function(Value)
        autoCollect = Value
...
local Afhubyyds = Tabs.Afhubyyds:AddLeftGroupbox("力量传奇付费功能公益", "rocket")
Afhubyyds:AddSlider('FlySpeedSlider', {
    Text = '飞行速度',
    Default = 5,
    Min = 1,
    Max = 20,
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            local controlModule = require(
                Players.LocalPlayer.PlayerScripts:WaitForChild("PlayerModule")
            ...
            local function setupFlight(char)
            ...
            getgenv().FlyLoop = RunService.RenderStepped:Connect(function()
                ...
                if char and hrp and hrp:FindFirstChild("VelocityHandler") and getgenv().fly then
                    char.Humanoid.PlatformStand = true
                    hrp.GyroHandler.CFrame = workspace.CurrentCamera.CFrame

                    local direction = controlModule:GetMoveVector()
                    local speed = flySpeed * flyMultiplier

                    hrp.VelocityHandler.Velocity = Vector3.zero

                    if direction.X ~= 0 then
                        hrp.VelocityHandler.Velocity += workspace.CurrentCamera.CFrame.RightVector * (direction.X * speed)
                    end
                    if direction.Z ~= 0 then
                        hrp.VelocityHandler.Velocity -= workspace.CurrentCamera.CFrame.LookVector * (direction.Z * speed)
                    end
                    if UserInputService:IsKeyDown(Enum.KeyCode.Space) then
                        hrp.VelocityHandler.Velocity += Vector3.new(0, speed / 2, 0)
                    end
...
Afhubyyds:AddToggle('NoclipToggle', {
    Text = '穿墙',
    Default = false,
    Tooltip = '开启后无视碰撞',
    Callback = function(Value)
...
Afhubyyds:AddToggle('AutoKillToggle', {
    Text = '自动击杀',
    Default = false,
    Tooltip = '开启后持续自动击杀',
    Callback = function(Value)
...
Afhubyyds:AddToggle('AutoPunchToggle', {
    Text = '自动挥拳',
    Default = false,
    Tooltip = '开启后持续自动挥拳',
    Callback = function(Value)
...
Afhubyyds:AddToggle('PositionLockToggle', {
    Text = '锁定坐标(位置)',
    Default = false,
    Tooltip = '锁定当前坐标防止移动',
    Callback = function(Value)
...
Afhubyyds:AddToggle('SwitchServerToggle', {
    Text = '自动换服',
    Default = false,
    Tooltip = '死亡或180秒后自动换服',
    Callback = function(Value)
        ...
        if Value then
            ...
            RunService.Heartbeat:Connect(function()
                if _G.switchServers and tick() - startTime >= 180 then
                    WindUI:Notify({
                        Title = "自动换服",
...
Afhubyyds:AddInput('StrengthInput', {
    Default = '',
    Numeric = true,
    Finished = true,
    Text = '修改力量',
    ...
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
            game.Players.LocalPlayer.leaderstats.Strength.Value = num
        end
    end
})

Afhubyyds:AddInput('RebirthsInput', {
    Default = '',
    Numeric = true,
    Finished = true,
    Text = '修改重生数',
    ...
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
            game.Players.LocalPlayer.leaderstats.Rebirths.Value = num
        end
    end
})

Afhubyyds:AddInput('KillsInput', {
    Default = '',
    Numeric = true,
    Finished = true,
    Text = '修改击杀数',
    ...
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
            game.Players.LocalPlayer.leaderstats.Kills.Value = num
        end
    end
})

Afhubyyds:AddInput('BrawlsInput', {
    Default = '',
    Numeric = true,
    Finished = true,
    Text = '修改获胜数',
    ...
    Callback = function(Value)
        local num = tonumber(Value)
        if num then
            game.Players.LocalPlayer.leaderstats.Brawls.Value = num
        end
    end
})
Afhubyyds:AddToggle('AutoRebirthToggle', {
    Text = '自动重生',
    Default = false,
    Tooltip = '开启后持续自动重生',
    Callback = function(Value)
...
Afhubyyds:AddButton({
    ...
    Func = function()
        game:GetService("ReplicatedStorage")
            .rEvents
            .changeSpeedSizeRemote:InvokeServer("changeSize", 2)

        Library:Notify("已设置为体积 2", 3)
    end,
    DoubleClick = false
})
Afhubyyds:AddToggle('AutoTrainTriple', {
    Text = '自动执行所有锻炼',
    Default = false,
    Tooltip = '自动执行所有健身训练',
    Callback = function(Value)
        ...
        spawn(function()
            while Value do
                ...
                for _, v in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
                    if v:IsA("Tool") and (v.Name == "Handstands"
                        or v.Name == "Situps"
                        or v.Name == "Pushups"
...
Afhubyyds:AddToggle('AutoWeight', {
    Text = '自动举哑铃',
    Default = false,
    Tooltip = '自动执行 Weight 训练',
    Callback = function(Value)
...
Afhubyyds:AddToggle('AutoPushup', {
    Text = '自动俯卧撑',
    Default = false,
    Tooltip = '自动执行 Pushups 训练',
    Callback = function(Value)
...
Afhubyyds:AddToggle('AutoSitup', {
    Text = '自动仰卧起坐',
    Default = false,
    Tooltip = '自动执行 Situps 训练',
    Callback = function(Value)
...
Afhubyyds:AddToggle('AutoHandstand', {
    Text = '自动倒立',
    Default = false,
    Tooltip = '自动执行 Handstands 训练',
    Callback = function(Value)
