...
local pingSimulated = 0
local executionMode = "正常"
local lastActionTime = 0
local actionCooldown = 0

local function updateMonitor(status, errMsg)
    if monitorLabel then
        ...
        if errMsg then
            ...
            monitorLabel.Text = string.format(
                "状态: %s | 错误: %d | %s\n%s",
                status, errorCount, timeStr, errMsg
            )
            monitorLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
        else
            monitorLabel.Text = string.format(
                "状态: %s | 错误: %d | 模式: %s\n运行正常 %s",
                status, errorCount, executionMode, timeStr
            )
...
local function simulatePing()
    pingSimulated = 0.05 + math.random() * 0.1
    return pingSimulated
end
...
local ghostBody = nil
local ghostAttachment = nil

local function createGhostPhysics(targetHRP)
    if ghostBody then 
        pcall(function() ghostBody:Destroy() end) 
    end
    if ghostAttachment then 
        pcall(function() ghostAttachment:Destroy() end) 
    end
    
    ghostAttachment = Instance.new("Attachment")
    ghostAttachment.Name = "QCAttachment"
    ghostAttachment.Parent = targetHRP.Parent or targetHRP
    
    ghostBody = Instance.new("VectorForce")
    ghostBody.Name = "QCVectorForce"
    ghostBody.Attachment0 = ghostAttachment
    ghostBody.Force = Vector3.new(0, 0, 0)
    ghostBody.Parent = targetHRP.Parent or targetHRP
    
    return true
end

local function quantumEntangle(myHRP, targetHRP)
    ...
    local impactForce = 5000 + math.random() * 3000
    local myVel = direction * impactForce
    
    myHRP.Velocity = myHRP.Velocity:Lerp(myVel, 0.15)
    
    if targetHRP.Velocity.Magnitude < 5 then
        if not targetHRP:FindFirstChild("QCVectorForce") then
            createGhostPhysics(targetHRP)
        end
        ...
        if ghostBody then
            ghostBody.Force = explosionForce
        end
        
        targetHRP.Velocity = targetHRP.Velocity + explosionForce.Unit * (500 + math.random() * 300)
        targetHRP.AssemblyLinearVelocity = targetHRP.Velocity
    end
    
    return true
end

local function safePositionCorrection(myHRP, targetHRP)
    ...
    local correctionForce = direction * (distance * 50)
    myHRP.AssemblyLinearVelocity = correctionForce
end

local function findPlayer(text)
    text = text:lower()
    for _, plr in pairs(Players:GetPlayers()) do
        if string.find(plr.Name:lower(), text) or 
           string.find(plr.DisplayName:lower(), text) then
            return plr
        end
    end
end
...
lp.Chatted:Connect(function(msg)
    ...
    if msg:sub(1,6):lower() == ";kill " then
        ...
        if plr and plr.Character and plr.Character:FindFirstChild("Humanoid") then
            ...
            executionMode = "锁定目标"
            updateMonitor("锁定", nil)
        else
            updateMonitor("运行中", "目标无效")
        end
    end
    ...
    if msg:lower() == ";status" then
        local info = string.format(
            "碰飞状态: %s | 目标: %s | 错误: %d",
            Enabled and "开启" or "关闭",
            targetPlayer and targetPlayer.Name or "无",
            errorCount
        )
        updateMonitor("运行中", info)
    end
    ...
    if msg:lower() == ";reset" then
        ...
        targetPlayer = nil
        executionMode = "正常"
        updateMonitor("重置", nil)
    end
end)

task.spawn(function()
    while task.wait(getRandomInterval()) do
        if not Enabled then 
            updateMonitor("待机", nil)
            continue 
        end
        
        if tick() - lastActionTime < actionCooldown then continue end
        
        if not targetPlayer then 
            updateMonitor("运行中", nil)
            continue 
        end
        ...
        if hum.Health <= 0 then
            ...
            targetPlayer = nil
            executionMode = "等待重生"
            updateMonitor("运行中", "目标已死亡")
            continue
        end

        local myChar = lp.Character
        if not myChar then
            updateMonitor("异常", "角色不存在")
            task.wait(1)
            continue
        end
        ...
        if myHRP and targetHRP then
            task.wait(simulatePing())
            
            local success = quantumEntangle(myHRP, targetHRP)
            
            safePositionCorrection(myHRP, targetHRP)
            
            lastActionTime = tick()
            actionCooldown = getRandomInterval() * 2
            
            if success then
                executionMode = "量子碰飞"
                updateMonitor("活跃", nil)
            else
                executionMode = "等待接近"
                updateMonitor("待机", nil)
            end
        else
            updateMonitor("异常", "HRP丢失")
        end
    end
end)
...
task.spawn(function()
    while task.wait(0.2) do
        ...
        if hum and hum.MoveDirection.Magnitude > 0 then
            hum:Move(Vector3.new(5000, 0, 5000))
        end
    end
end)

local screenGui = Instance.new("ScreenGui")
screenGui.Name = "FlyOffGUI_V4"
...
toggleBtn.Position = UDim2.new(0.5, -90, 0, 34)
...
monitorLabel.Size = UDim2.new(1, -10, 0, 60)
monitorLabel.Position = UDim2.new(0, 5, 0, 72)
monitorLabel.BackgroundTransparency = 1
monitorLabel.Text = "状态: 待机 | 错误: 0 | 模式: 正常\n运行正常"
monitorLabel.TextColor3 = Color3.fromRGB(100, 255, 100)
monitorLabel.TextSize = 10
...
infoLabel.Text = ";kill 玩家 | ;status 状态 | ;reset 重置"
...
closeBtn.Size = UDim2.new(0, 180, 0, 24)
closeBtn.Position = UDim2.new(0.5, -90, 0, 152)
...
toggleBtn.MouseButton1Click:Connect(function()
    ...
    if Enabled then
        ...
        targetPlayer = nil
        originalCFrame = nil
        if ghostBody then pcall(function() ghostBody:Destroy() end) ghostBody = nil end
        if ghostAttachment then pcall(function() ghostAttachment:Destroy() end) ghostAttachment = nil end
        updateMonitor("关闭", nil)
    end
end)
