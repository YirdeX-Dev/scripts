...
local Bhgn = Tabs.Main:AddLeftGroupbox("拔河", "git-commit-horizontal")
...
Afmsktb:AddButton({
    ...
    Func = function()
        ...
        local success, err = pcall(function()
            local sugarObj = getNil("SugarObject", "RemoteEvent") or getNil("SugarGame", "RemoteEvent")            
            if sugarObj then
                local args = {
                    {
                        Completed = true,
                ...
                sugarObj:FireServer(unpack(args))
                local altEvent = getNil("FinishEvent", "RemoteEvent")
                if altEvent then
                    altEvent:FireServer(true)
                end
                local antiDeath = getNil("DeathChecker", "RemoteEvent")
                if antiDeath then
                    antiDeath:FireServer(false)
                end
                
                return true
            else
                return false, "未找到糖饼相关事件"
            end
        end)
        if success and err == true then
            Library:Notify("糖饼已完成！成功过关！", 3)
        elseif success and type(err) == "string" then
            Library:Notify("" .. err, 3)
        else
            Library:Notify("完成失败，请重试", 3)
        end
    end,
    DoubleClick = false,
})
Afmsktb:AddButton({
    ...
    Func = function()
        ...
        local methods = {
            function()
                local ev = getNil("SugarEvent", "RemoteEvent")
                if ev then
                    ev:FireServer({Completed = true, Action = "Skip"})
                    return true
                end
            end,
            function()
                local ev = getNil("SugarProgress", "RemoteEvent")
                if ev then
                    ev:FireServer({Progress = 100, Complete = true})
                    return true
                end
            end,
            function()
                local ev = getNil("GameRemote", "RemoteEvent")
                if ev then
                    ev:FireServer("CompleteSugar", true)
                    return true
                end
            end,
            function()
                ...
                if humanoid then
                    humanoid.Health = 100
                    wait(0.1)
                    local sugarCheck = character:FindFirstChild("SugarCompleted")
                    if sugarCheck then
                        sugarCheck.Value = true
                    end
                    return true
                end
            end
        ...
        for i, method in ipairs(methods) do
            ...
            if success then
                completed = true
                Library:Notify("方法" .. i .. "成功执行", 2)
                break
            end
        end        
        if completed then
            Library:Notify("糖饼完成！已绕过检测！", 3)
        else
            Library:Notify("所有方法均失败，可能需要更新", 3)
        end
    end,
    DoubleClick = false,
})
...
Zdxg:AddDropdown('GunSelector', {
    ...
    Multi = true,
    Text = '选择要修改的枪械',
    Tooltip = '可多选，选中的枪械子弹会被修改',
    Callback = function(Value)
        _G.SelectedGuns = Value
    end
})
...
Zdxg:AddButton('修改子弹', function()
    ...
    local Selected = _G.SelectedGuns or {}
    ...
    for GunName, IsSelected in pairs(Selected) do
        if IsSelected then
            local Gun = Backpack:FindFirstChild(GunName)
            if Gun then
                local InfoClient = Gun:FindFirstChild("InfoClient")
                if InfoClient and InfoClient:FindFirstChild("Bullets") then
                    InfoClient.Bullets.Value = Amount
                    print('[成功] ' .. GunName .. ' 子弹已改为 ' .. Amount)
                else
                    print('[警告] ' .. GunName .. ' 找不到 InfoClient.Bullets')
                end
            else
                print('[警告] 背包中没有 ' .. GunName)
            end
        end
    end
end)
